...
const { listarCitas, obtenerCita, crearCita, cancelarCita, reprogramarCita, confirmarCita } = require('../controllers/citas.controller');
const { verificarToken, verificarRol } = require('../middlewares/auth.middleware');
const { validarTexto } = require('../utils/validators');

const router = Router();

const ROLES_CITAS = ['admin', 'medico', 'enfermera', 'recepcionista'];
...
const validarReprogramar = [
  body('fecha').isDate().withMessage('Fecha inválida.'),
  body('hora_inicio').matches(/^\d{2}:\d{2}$/).withMessage('Hora de inicio inválida (HH:MM).'),
  validarTexto('motivo', 'Motivo', { opcional: true, maxLen: 500 }),
];
...
router.post('/', verificarToken, verificarRol('admin', 'recepcionista', 'medico'), validarCrear, crearCita);
